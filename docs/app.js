...
const state = {
  site: { title: '公开岗位线索池', subtitle: '静态结果看板', storageKey: 'public-job-leads-template-v2' },
  leads: [],
...
function renderLeads() {
  ...
  const roleFamily = document.getElementById('roleFamily').value;
  ...
  const filtered = state.leads
    .filter(item => jobType === 'all' || item.jobType === jobType)
    .filter(item => roleFamily === 'all' || item.roleFamily === roleFamily)
    ...
    .filter(item => !query || [item.company, item.title, item.jobType, item.roleFamily, item.platform, item.summary, item.trustReason, ...item.risks].join(' ').toLowerCase().includes(query))
  ...
  body.innerHTML = filtered.length ? filtered.map(item => {
    ...
    return `<tr class="${rowClasses}">
      ...
      <td><div class="title">${esc(item.company)}</div><div>${esc(item.title)}</div><div class="sub">${esc(item.jobType)} · ${esc(item.roleFamily)}</div></td>
      <td>${esc(item.city)}</td>
...
function exportCsv() {
  const rows = [['可信度', '开放状态', '公司', '岗位', '机会类型', '岗位方向', '城市', '平台', '发布日期', '核验日期', '摘要', '风险', '原帖', '我的状态']];
  state.leads.forEach(item => {
    const personal = state.saved[item.id] || { status: 'todo' };
    rows.push([item.trust, postingLabels[item.status], item.company, item.title, item.jobType, item.roleFamily, item.city, item.platform, item.date, item.checked, item.summary, item.risks.join('；'), item.url, personalLabels[personal.status]]);
  });
  const csv = '\ufeff' + rows.map(row => row.map(value => `"${String(value ?? '').replace(/"/g, '""')}"`).join(',')).join('\r\n');
  const link = document.createElement('a');
  link.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8' }));
  link.download = `公开岗位线索_${new Date().toISOString().slice(0, 10)}.csv`;
  link.click();
...
async function init() {
  try {
    ...
    loadSavedState();
    populateSelect('jobType', uniqueSorted(state.leads.map(item => item.jobType)), '全部机会类型');
    populateSelect('roleFamily', uniqueSorted(state.leads.map(item => item.roleFamily)), '全部岗位方向');
...
['query', 'jobType', 'roleFamily', 'city', 'platform', 'trust', 'postingStatus'].forEach(id => {
